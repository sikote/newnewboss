...
async def LadderFunc(number, ladderlist, channelVal):
	...
	embed = discord.Embed(title  = "🎲 あみだあみだ",color=0x00ff00)
	embed.add_field(name = "👥 参加者", value =  f"```fix\n{', '.join(ladderlist)}```", inline=False)
	embed.add_field(name = "😍 winner", value =  f"```fix\n{', '.join(result_ladder)}```")
	embed.add_field(name = "😭 loser", value =  f"```{', '.join(lose_member)}```")
							...
							if basicSetting[2] != '0':
								if int(basicSetting[17]) <= bossMungCnt[i] and int(basicSetting[17]) != 0:
									...
									if bossData[i][2] == '0':
										...
										await self.bot.get_channel(channel).send("```" +  bossData[i][0] + ' 未入力```', tts=False)
										embed = discord.Embed(
											description= '```next ' + bossData[i][0] + ' ' + bossTimeString[i] + 'desu```',
											color=0xff0000
											)
										await self.bot.get_channel(channel).send(embed=embed, tts=False)
										try:
	...
	async def on_message(self, msg):
		...
		if chflg == 1 :
			if self.get_channel(basicSetting[7]).id == msg.channel.id:
				...
				for i in range(bossNum):
					################ 보스 컷처리 ################ 
					if message.content.startswith(bossData[i][0] +'cut') or message.content.startswith(convertToInitialLetters(bossData[i][0] +'컷')) or message.content.startswith(bossData[i][0] +' 컷') or message.content.startswith(convertToInitialLetters(bossData[i][0] +' 컷')):
						if hello.find('  ') != -1 :
					...
					if message.content.startswith(bossData[i][0] +'pass') or message.content.startswith(bossData[i][0] +' pass'):
						if hello.find('  ') != -1 :
							bossData[i][6] = hello[hello.find('  ')+2:]
							hello = hello[:hello.find('  ')]
						else:
